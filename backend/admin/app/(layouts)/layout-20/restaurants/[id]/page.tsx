...
export default function RestaurantDetailsPage() {
  ...
  async function loadRestaurant() {
    ...
    try {
      const data = await apiGet<RestaurantDetails>(`${API_URL}/restaurants/${id}`);
      setRestaurantDetails(data);
    } catch (e: any) {
      setRestaurantErr(e?.message || 'Ошибка загрузки ресторана');
    } finally {
  ...
  async function uploadCover() {
    if (!coverFile) {
      setCoverUploadMsg('Сначала выбери файл');
      return;
    }
    ...
    try {
      ...
      const updated = await apiUpload<RestaurantDetails>(
        `${API_URL}/restaurants/${id}/cover`,
        formData,
      );
      ...
      setCoverPreview('');
      setCoverUploadMsg('Фото ресторана загружено');
    } catch (e: any) {
      setCoverUploadMsg(e?.message || 'Ошибка загрузки фото');
    } finally {
    ...
    <div className="space-y-5 rounded-2xl p-4 md:p-5 bg-slate-50">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
        <Panel
          ...
          subtitle="Рекомендуемый формат: 16:9. Лучший размер: 1200x675."
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-3">
              <button
                ...
                style={{ borderRadius: 12 }}
              >
                {restaurantDetails?.coverImageUrl ? 'Заменить фото' : 'Выбрать фото'}
              </button>
              ...
              <button
                ...
                style={{ borderRadius: 12 }}
              >
                {coverUploading ? 'Загрузка...' : 'Загрузить фото'}
              </button>
            </div>

            <div className="text-sm text-slate-500">
              Поддерживаются: JPG, JPEG, PNG, WEBP. Максимум: 10 MB.
            </div>
        ...
        <Panel
          ...
          subtitle={
            <div className="space-y-2">
              ...
              <div>№ ресторана: {restaurantDetails?.number || metrics?.restaurant?.number || '—'}</div>
              <div>Телефон: {restaurantDetails?.phone || '—'}</div>
              <div>Адрес: {restaurantDetails?.address || '—'}</div>
              <div>Часы работы: {restaurantDetails?.workingHours || '—'}</div>
            </div>
          }
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="rounded-xl border border-slate-200 p-4 bg-black/[0.02]">
              <div className="text-base opacity-70">Описание RU</div>
            ...
            <div className="rounded-xl border border-slate-200 p-4 bg-black/[0.02]">
              <div className="text-base opacity-70">Описание KK</div>
              <div className="text-lg font-semibold mt-2 whitespace-pre-wrap">
            ...
            <div className="rounded-xl border border-slate-200 p-4 bg-black/[0.02]">
              ...
              <div className="text-base opacity-70 mt-2">
                override: {restaurantDetails?.restaurantCommissionPctOverride ?? '—'}
              </div>
            </div>

            <div className="rounded-xl border border-slate-200 p-4 bg-black/[0.02]">
              <div className="text-base opacity-70">Показ на главной</div>
              ...
              <div className="flex items-center gap-3 mt-3">
                <input
                  ...
                  onChange={async (e) => {
                    ...
                    try {
                      const r = await fetch(`${API_URL}/restaurants/${id}/pinned`, {
                        method: 'PATCH',
                        credentials: 'include',
                        headers: {
                      ...
                      alert('Ошибка обновления pinned');
                    }
                  }}
                />

                <span className="text-lg font-semibold">
                  {restaurantDetails?.isPinned ? 'На главной' : 'Не показывается'}
                </span>
              </div>

              <div className="text-sm text-slate-500 mt-2">
                Если включено — ресторан будет показываться на главной странице приложения
              </div>
            </div>
          </div>
        </Panel>
