...
function setRefreshCookie(token: string) {
  document.cookie = `refresh_token=${encodeURIComponent(token)}; Path=/; SameSite=Lax`;
}
...
export default function LoginPage() {
  ...
  // DEV: логин без пароля, но с настоящими JWT токенами
  useEffect(() => {
    if (!DISABLE_AUTH) return;

    (async () => {
      try {
        const r = await fetch(`${API_URL}/auth/dev-admin-token`, {
          method: "POST",
        });
        ...
        if (!r.ok) {
          throw new Error(data?.message || "Не смог получить dev токен");
        }

        if (!accessToken || !refreshToken) {
          throw new Error("Dev login response does not contain both tokens");
        }
  ...
  return (
    <div className="max-w-md space-y-4">
      ...
      {msg && <div className="text-red-600 text-sm">{msg}
        </div>}
    </div>
  );
}